...
async function startServer() {
  ...
  app.get('/api/admin/doctor-attendance', authenticateJWT, requireRole(['ADMIN']), async (req, res) => {
    try {
      ...
      // Fetch all activity logs in the padded range
      ...
      // Fetch all consultations completed in the padded range
  ...
  app.post('/api/activity-logs', authenticateJWT, requireRole(['ADMIN', 'RECEPTION', 'DOCTOR', 'PHARMACY']), async (req, res) => {
    const { action, user, details, userId } = req.body;
    try {
      const resolvedUserId = userId ? String(userId) : ((req as any).user?.userId ? String((req as any).user.userId) : undefined);
  ...
  app.get('/api/bills', authenticateJWT, requireRole(['PHARMACY', 'ADMIN']), async (req: any, res: any) => {
    const { startDate, endDate } = req.query;
    try {
      ...
      if (startDate || endDate) {
        ...
        if (startDate) {
          ...
          // Widen by 36 hours to safely fetch all timezone candidates
          const startWider = new Date(start.getTime() - 36 * 60 * 60 * 1000);
          whereClause.createdAt.gte = startWider;
        }
        if (endDate) {
          const end = new Date(String(endDate));
          end.setHours(23, 59, 59, 999);
          // Widen by 36 hours to safely fetch all timezone candidates
          const endWider = new Date(end.getTime() + 36 * 60 * 60 * 1000);
          whereClause.createdAt.lte = endWider;
        }
      }
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (status === 'PAID' && currentBill.status !== 'PAID') {
          ...
          for (const item of currentBill.items) {
            ...
            // 2. Fetch all active, non-expired batches for this medicine
            // Non-expired means expiryDate is either null or >= today
            const activeBatches = await tx.inventoryItem.findMany({
              where: {
            ...
            // 3. Calculate total stock across active non-expired batches
            ...
            // 4. Deduct stock sequentially (FEFO)
  ...
  app.post('/api/inventory/add-stock', authenticateJWT, requireRole(['PHARMACY', 'ADMIN']), async (req: any, res: any) => {
    ...
    try {
      ...
      const newBatch = await prisma.inventoryItem.create({
        data: {
          itemCode,
  ...
  app.post('/api/inventory/bulk-import', authenticateJWT, requireRole(['PHARMACY', 'ADMIN']), async (req, res) => {
    try {
      ...
      for (const raw of items) {
        ...
        // Find existing or update/create
        const existing = await prisma.inventoryItem.findUnique({
