...
Emergency Contact: ${formData.emergencyContactName} (${formData.emergencyContactPhone})
  ...
  const handleLookupSearch = () => {
    ...
    const matched = list.find(p => 
      (p.id.toLowerCase().includes(lookupQuery.toLowerCase()) && lookupQuery.trim() !== '') ||
      (p.phone.includes(lookupPhone) && lookupPhone.trim() !== '') ||
      (p.name.toLowerCase().includes(lookupQuery.toLowerCase()) && lookupQuery.trim() !== '')
    );
