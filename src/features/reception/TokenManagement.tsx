...
export default function TokenManagement() {
  ...
  const handleRefreshSync = async () => {
    ...
    try {
      await Promise.all([fetchPatients(), fetchTokens(), fetchUsers()]);
      setIsSyncing(false);
      toast.dismiss();
  ...
  const handleExportPatientDossier = async (patientId: string, currentPatientName?: string) => {
    try {
      ...
      // 2. Fetch all bills from state
      await useStore.getState().fetchBills();
      const allBills = useStore.getState().bills || [];
      ...
      generateFullPatientHistoryPDF(patientObj, historyRes, allBills, allTokens);
  ...
  const handleExportDateWiseTokens = async () => {
    try {
      toast.info("Analyzing and compiling historical queue logs...");
      
      // Fetch fresh tokens so we have latest DB snapshot
      await useStore.getState().fetchTokens();
      const allTokens = useStore.getState().tokens || [];
  ...
  const handleExportAllPatientsSystemWide = async (format: 'pdf' | 'csv') => {
    try {
      ...
      const { fetchPatients, fetchBills, fetchTokens } = useStore.getState();
      await Promise.all([fetchPatients(), fetchBills(), fetchTokens()]);
      
      const allPatients = useStore.getState().patients || [];
      const allBills = useStore.getState().bills || [];
      const allTokens = useStore.getState().tokens || [];
