...
export default function PharmacyDashboard() {
  ...
  // Reactive fetchBills aligned dynamically with date-range filters (database-level query)
  useEffect(() => {
    if (dateRangeActive && filterStartDate && filterEndDate) {
      fetchBills(filterStartDate, filterEndDate);
    } else {
      fetchBills();
    }
  }, [filterStartDate, filterEndDate, dateRangeActive, fetchBills]);
  ...
  const filteredBillsList = React.useMemo(() => {
    return (bills || []).filter((b: any) => {
      ...
      // Local date range filtering safely converting all timezones with midnight boundaries
      if (dateRangeActive && filterStartDate && filterEndDate) {
        ...
        if (bDate < startLimit || bDate > endLimit) {
          return false;
        }
      }
  ...
  }, [bills, billingFilter, selectedMedFilter, searchTerm, dateRangeActive, filterStartDate, filterEndDate, patients]);
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          ...
          {activeTab === 'billing' && (
            <div className="space-y-6 animate-fade-in">
              ...
              {/* Bill Stats Section */}
              {(() => {
                const comBillsForStats = filteredBillsList.filter((b: any) => b.status === 'PAID');
                const revenueVal = comBillsForStats.reduce((sum: number, b: any) => sum + parseFloat(b.total || '0'), 0);
                const totalDispToday = comBillsForStats.length;
                const pendingClearVal = filteredBillsList.filter((b: any) => {
                  const isUnpaid = b.status === 'UNPAID';
                  const isFlagged = b.status === 'FLAGGED' || (b.items || []).some((it: any) => it.name.toUpperCase().includes('OXYCODONE'));
                  return isUnpaid || isFlagged;
                }).length;
              ...
              {(() => {
                ...
                const totalBillPages = Math.ceil(filteredBillsList.length / billsPageSize) || 1;
                const paginatedBillsList = filteredBillsList.slice(
                  (billingPage - 1) * billsPageSize,
                  billingPage * billsPageSize
                );
                  ...
                  <div className="bill-table-wrap">
                    ...
                    <div className="bill-pg">
                      <span className="font-semibold text-slate-500 text-xs">
                        Showing {Math.min(filteredBillsList.length, (billingPage - 1) * billsPageSize + 1)}-
                        {Math.min(filteredBillsList.length, billingPage * billsPageSize)} of {filteredBillsList.length} total transactions
                      </span>
